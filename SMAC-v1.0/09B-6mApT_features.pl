...
$num_AT=0;
while (defined ($SD=<IN5>)){
	if ($SD=~/(.+)/){
		$SD{$1}=1}}
while (defined ($A=<IN1>)){
	if ($A=~/^(\d+)\s+/ && !exists $SD{$1}){
	$num_A++;}}
while (defined ($A=<IN2>)){
	if ($A=~/(\d+)\s+(.+?)\s+(\d+)\s+(.)\s+(.+)\s+AT\s+/ && !exists $SD{$1}){
		$pos=join "~","$1","$2","$3","$4";
		$hash{$pos}=$5;
		}}
while (defined ($A=<IN3>)){
...
while (defined ($A=<IN4>)){
	...
	elsif ($A=~/(\d+)\s+(.+?)\s+(\d+)\s+C\s+(.+)\s+AT\s+(.+)/ && !exists $SD{$1}){
                $pos=$3-1;
                $pos1=join "~","$1","$2","$pos","W";
                if (exists $hash{$pos1}){
